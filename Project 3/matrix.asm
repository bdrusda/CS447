...
speed: .space 400	#array holding 100 column speeds
...
la $s2, speed
li $t0, 0	#counter
li $t1, 11	#iteration counter (+11)
update_columns:
	#pass head and speed as params
	add $a0, $zero, $s1	#head
	add $a1, $zero, $s2	#speed

	lw $t2, 0($s2)		#load the speed of the current column
	divu $t1, $t2		#see if this is a multiple of the iteration number
	mfhi $t2		#get the remainder
	bne $t2, 0, no_update	#if the remainder wasn't 0, don't update
	
	
	addi $sp, $sp, -8	#store counters
	sw $t0, 0($sp)	
	sw $t1, 4($sp)
	jal update_indiv
	lw $t1, 4($sp)
	lw $t0, 0($sp)		#retrieve counters
	addi $sp, $sp, 8
	
	no_update:
	
	addi $t0, $t0, 1	#increment counter
	addi $t1, $t1, 1	#increment iteration number
...
#set column speed
	...
	li $a1, 5
	li $v0, 42
	syscall			#generate a random speed up to 5
	
	add $t2, $zero, $a0	#store the speed in t2
	addi $t2, $t2, 1	#remove the possibility of 0
...
update_color_and_letter:
	...
	li $a1, 93
	li $v0, 42
	syscall			#generate a random character
	...
	#black out the previous character
	li $t2, 0x00000000
	addi $t3, $s0, -320
	sw $t2, 0($t3)
